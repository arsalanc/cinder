...
function updateCreatures() {
  for (let i = creatures.length - 1; i >= 0; i--) {
    ...
    // tempest handles its own fire discipline in the storm branch below)
    if (t.ranged && !t.storm) {
      c.attackCd--;
    ...
    if (t.burrow) {
      ...
      if (c.surgeT > 0) {
        ...
        continue; // dug in deep: no contact, no trail, invulnerable
      }
      ...
      if (c.slamT > 0) { c.slamT--; continue; } // dazed a beat after a dry slam
    ...
    if (t.storm) {
      ...
      if (c.squallT > 0) {
        ...
        continue; // storm-wreathed: invulnerable until the squall passes
      }
